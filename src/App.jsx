...
function downloadListingPoster(listing, lang) {
  const shareUrl = getListingShareUrl(listing);
  const title = listing?.title?.[lang] || listing?.title || "Campiq housing";
  const area = listing?.area || listing?.address || "Campiq";
  const price = listing?.price ? `$${listing.price}/mo` : "View details";
...
<svg xmlns="http://www.w3.org/2000/svg" width="1080" height="1440" viewBox="0 0 1080 1440">
  ...
  <rect width="1080" height="1440" rx="48" fill="url(#bg)"/>
  <rect x="90" y="96" width="900" height="1248" rx="42" fill="#ffffff"/>
  <circle cx="168" cy="176" r="42" fill="#f97316"/>
  <text x="228" y="170" font-family="Arial, sans-serif" font-size="44" font-weight="800" fill="#18181b">Campiq</text>
  <text x="228" y="214" font-family="Arial, sans-serif" font-size="24" fill="#71717a">Student housing, roommates, and campus life</text>
  <text x="120" y="390" font-family="Arial, sans-serif" font-size="64" font-weight="900" fill="#18181b">${xmlEscape(title).slice(0, 70)}</text>
  <text x="120" y="485" font-family="Arial, sans-serif" font-size="44" font-weight="800" fill="#ea580c">${xmlEscape(price)}</text>
  <text x="120" y="545" font-family="Arial, sans-serif" font-size="32" fill="#52525b">${xmlEscape(area)}</text>
  <rect x="120" y="625" width="840" height="300" rx="36" fill="#fff7ed"/>
  <text x="170" y="735" font-family="Arial, sans-serif" font-size="40" font-weight="800" fill="#18181b">Open this listing on Campiq</text>
  <text x="170" y="795" font-family="Arial, sans-serif" font-size="28" fill="#71717a">Scan the QR code or open the shared link.</text>
  <image href="${qrUrl}" x="360" y="980" width="360" height="360"/>
  <text x="540" y="1380" text-anchor="middle" font-family="Arial, sans-serif" font-size="24" fill="#71717a">${xmlEscape(shareUrl)}</text>
</svg>`;
  const blob = new Blob([svg], { type: "image/svg+xml" });
  const url = URL.createObjectURL(blob);
  const link = document.createElement("a");
  link.href = url;
  link.download = `campiq-listing-${listing?.id || "share"}.svg`;
  link.click();
        ...
        <div className="grid gap-6 p-5 sm:grid-cols-[1fr_240px] sm:p-6">
          <div className="space-y-4">
            ...
            <div className="rounded-3xl border bg-white p-4">
              ...
              <p className="mt-2 text-sm text-zinc-600">{lang === "en" ? "Download a poster. The QR code opens this listing." : "下载海报，海报里的二维码可以打开这个房源。"}</p>
              <Button type="button" variant="orange" className="mt-4" onClick={() => downloadListingPoster(listing, lang)}>{lang === "en" ? "Download poster" : "下载海报"}</Button>
            </div>
          </div>
...
export default function CampiqWebsite() {
  ...
  if (page === "housing") return <HousingPage lang={lang} setLang={setLang} t={t} setPage={setPage} listings={listings} onSelectListing={openListing} />;
